% This is 3 layer neural next work for group3

n=3;
lambda=0;
train=group3(1:ceil(length(group3)*2/3),:)';
y1_train=y3(1:ceil(length(group3)*2/3))';


test=group3(ceil(length(group3)*2/3):length(group3),:)';
y1_test=y3(ceil(length(group3)*2/3):length(group3))';
...
group3inerror=Ein1
group3outerror=Eout1
group3classtrain=xL;
group3classtest=xval1;
group3train=train;
group3test=test;
group3_y_train=y1_train;
group3_y_test=y1_test;

group3_1=w3_1;
group3_2=w3_2;
group3_3=w3_3;
base_rate3=sum(y3==1)/length(y3);
