% This is 3 layer neural next work for group4

n=3;
lambda=0;
train=group4(1:ceil(length(group4)*2/3),:)';
y1_train=y4(1:ceil(length(group4)*2/3))';


test=group4(ceil(length(group4)*2/3):length(group4),:)';
y1_test=y4(ceil(length(group4)*2/3):length(group4))';
...
g3=0*w3_3;
p2=500000;%iteration upper bound
...
group4inerror=Ein1
group4outerror=Eout1
group4classtrain=xL;
group4classtest=xval1;
group4train=train;
group4test=test;
group4_y_train=y1_train;
group4_y_test=y1_test;

group4_1=w3_1;
group4_2=w3_2;
group4_3=w3_3;

base_rate4=sum(y4==1)/length(y4)
